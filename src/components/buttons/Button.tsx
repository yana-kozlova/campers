...
export const Button: React.FC<ButtonProps> = ({ variant, children, onClick }) => {
    ...
    const buttonStyles = {
        ...
        '&:hover': {
            border:  variant === 'primary' ? 'none' : `1px solid ${eColors.PRIMARY_LIGHT}`,
            backgroundColor: variant === 'primary' ? eColors.PRIMARY_LIGHT : eColors.WHITE,
        },
    };
    
    return (
        <MuiButton style={buttonStyles} onClick={onClick}>
            {children}
        </MuiButton>
    );
};
