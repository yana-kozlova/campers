...
import { Avatar, Paper, Rating, Stack, styled, Typography } from '@mui/material';
...
const ReviewerAvatar = styled(Avatar)({
    backgroundColor: eColors.SILVER, width: 60, height: 60, color: eColors.PRIMARY, fontSize: '24px', fontWeight: 600
});
...
export const CamperReviewCard: React.FC<ICamper> = (props) => {
    ...
    return (<ReviewCard key={id} elevation={0}>
        <Stack direction="column" spacing={5}>
            {reviews.map((review, index) => (<Stack key={index} direction="column" sx={{width: '100%'}} spacing={3}>
                ...
                <Typography variant="h5" color="secondary">{review.comment}</Typography>
            </Stack>))}
        </Stack>
    </ReviewCard>);
};